...
* Remove all punctuation except apostrophes and dashes
* Fix encoding issues
* How do I find encoding issues?
* Could break into separate lines on each period

2. The data contains lots of times, dates, numbers and currency values. How to handle these? Are they useful for prediction?
* Remove numbers, because they aren't good candidates for prediction

3. How do you find typos in the data?
* Ignore them. The dataset should be big enough to provide the right spellings. Typos in input will lead to poorer predictions.

4. How do you identify garbage, or the wrong language?
* Ignore it - it won't occur very, so shouldn't affect the predictions

5. How do you define profanity? How do you ensure you don't remove words you want to include?
* Keep a separate profanity list, and load it in and remove it
...
* 3-grams is probably a good approach
* We can experiment with different n-grams
* Part of speech tagging would be an interesting approach
* We could implement a cache to improve accuracy
* Could use a bag of words approach (TermDocMatrix), that looks for words in a sentence
